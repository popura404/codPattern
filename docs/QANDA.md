...
2. 点位是否放在火、岩浆、仙人掌等危险位置
3. 点位上方是否被方块、流体或碰撞体占住
4. 点位维度是否和地图维度一致
...
## 10. 为什么 `endtp` 命令会提示地图名歧义

因为 `endtp` 子命令只按地图名查找，不带模式参数。

如果 `frontline` 和 `teamdeathmatch` 下存在同名地图，`show` / `set` / `clear` 都会报歧义。
