...
final class CodTdmVoteCoordinator {
    ...
    private final Supplier<CodTdmMap.GamePhase> phaseSupplier;
    ...
    CodTdmVoteCoordinator(
            ...
            Supplier<CodTdmMap.GamePhase> phaseSupplier,
    ...
    boolean setPlayerReady(ServerPlayer player, boolean ready) {
        if (phaseSupplier.get() != CodTdmMap.GamePhase.WAITING) {
            return false;
        }
    ...
    boolean voteToStart(UUID playerId) {
        return voteService.initiateStartVote(playerId);
    }

    boolean voteToEnd(UUID playerId) {
        return voteService.initiateEndVote(playerId);
    }
